...
class GameParam(QObject):
    ...
    _pieces_dropped_changed = Signal(int)# 累计下落部件更新
    ...
    def reset(self) -> None:
        ...
        self.set_score(0)# 分数
        self.set_level(1)# 等级
        self.set_lines_removed(0)# 累计移除行数
        self.set_pieces_dropped(0)# 累计下落部件

    def set_score(self, value: int) -> None:
        '''设置分数
        '''
        self._score = value
        self._score_changed.emit(value)

    def set_level(self, value: int) -> None:
        '''设置等级
        '''
        self._level = value
        self._level_changed.emit(value)

    def set_lines_removed(self, value: int) -> None:
        '''设置累计移除行数
        '''
        self._lines_removed = value
        self._lines_removed_changed.emit(value)

    def set_pieces_dropped(self, value: int) -> None:
        '''设置累计下落部件
        '''
        self._pieces_dropped = value
        self._pieces_dropped_changed.emit(value)
...
class GameWork(QObject):
    ...
    def update_curr_piece(self) -> bool:
        ...
        self._curr_x = (self.__board.get_width() // 2) + 1# 部件底部中点坐标
        self._curr_y = (self.__board.get_height() - 1) + self._curr_piece.get_bottom_y()
        # 不可放置
        if not self.__board.is_piece_setable(self._curr_piece, self._curr_x, self._curr_y):
            return False
        # 下一部件
        self._next_piece.set_random_shape()
        self._next_piece_changed.emit()# 下一部件更新
    ...
    def move_curr_piece(self, dx: int, dy: int) -> bool:
        ...
        # 不可放置
        if not self.__board.is_piece_setable(self._curr_piece, target_x, target_y):
            return False
        # 更新
    ...
    def rotate_curr_piece(self) -> bool:
        ...
        # 不可放置
        if not self.__board.is_piece_setable(new_piece, self._curr_x, self._curr_y):
            return False
        # 更新
...
class TetrisGame(QWidget):
    ...
    def __init_param(self) -> None:
        '''控制参数
        '''
        self._param = GameParam()
        self._param._score_changed.connect(self.__ui.lcdNumber_score.display)# 分数更新
        self._param._level_changed.connect(self.__ui.lcdNumber_level.display)# 等级更新
        self._param._lines_removed_changed.connect(self.__ui.lcdNumber_removed_lines.display)# 累计移除行数更新
        # self._param._pieces_dropped_changed.connect(self.__ui.lcdNumber_score.display)# 累计下落部件更新

    def __init_work(self) -> None:
        '''运行
        '''
        # 面板
        self.__board = TetrisBoard()# 面板
        # 部件
    ...
    def update_piece(self) -> None:
        ...
        # 位置
        self._curr_x = (self.__board.get_width() // 2) + 1
        self._curr_y = (self.__board.get_height() - 1) + self._curr_piece.get_bottom_y()
    ...
    def start(self) -> None:
        ...
        if self._state == GameState.End:# 必须在结束状态
            # 参数
            self._param.reset()# 重置参数
            # 界面
            self.__board.clear()# 清空面板
    ...
    def draw_square(self, painter: QPainter, x: int, y: int, square: Square):
        ...
        '''
        # 方块
        square_width = self.contentsRect().width() / self.__board.get_width()
        square_height = self.contentsRect().height() / self.__board.get_height()
